...
fn loom_concurrent_send_at_different_gpas() {
    loom::model(|| {
        ...
        let (h_a, _, _) =
            Capability::<Domain>::carve(&root, r0_h, Access::new(0x0000, 0x1000, Rights::RW))
                .unwrap();
        let (h_b, _, _) =
            Capability::<Domain>::carve(&root, r0_h, Access::new(0x2000, 0x1000, Rights::RW))
                .unwrap();
        ...
        let ta = thread::spawn(move || {
            ...
            Capability::<Domain>::send_at(&r, h_a, dom_h, Attributes::NONE, Some(0xA_0000))
        });
        ...
        let tb = thread::spawn(move || {
            let _guard = pl.read().unwrap();
            Capability::<Domain>::send_at(&r, h_b, dom_h, Attributes::NONE, Some(0xB_0000))
        });
...
fn loom_concurrent_send_at_same_gpa() {
    loom::model(|| {
        ...
        let tb = thread::spawn(move || {
            let _guard = pl.read().unwrap();
            Capability::<Domain>::send_at(&r, h_b, dom_h, Attributes::NONE, Some(0xA_0000))
        });
...
fn loom_concurrent_accept_at() {
    loom::model(|| {
        ...
        Capability::<Domain>::send_at(&root, c_h, dom_h, Attributes::NONE, Some(0xA_0000))
            .unwrap();
        ...
        let ta = thread::spawn(move || {
            let _guard = pl.read().unwrap();
            Capability::<Domain>::accept_at(&d, pid, None)
        });
...
fn loom_revoke_vs_accept_at_map_consistency() {
    loom::model(|| {
        ...
        Capability::<Domain>::send_at(&root, c_h, dom_h, Attributes::NONE, Some(0xC_0000))
            .unwrap();
        ...
        let revoker = thread::spawn(move || {
            ...
            Capability::<Domain>::revoke(&r, r0_h, c_sub)
        });
        ...
        let acceptor = thread::spawn(move || {
            ...
            Capability::<Domain>::accept_at(&d, pid, Some(0xC_0000))
        });
