...
fn test_complex_memory_update_scenario() {
    ...
    let (r1_h, carve_updates) = Capability::carve_memory(&dom0, r0_h, r1_access).unwrap();
    ...
    let (r2_h_in_dom1, carve2_updates) =
    ...
    let r3_h_in_dom1 = Capability::alias_memory(&dom1, r2_h_in_dom1, r3_access).unwrap();
    ...
    // r3_h_in_dom1 is r3's stable SubHandle in r2's capability tree.
    // ================================================================
    println!("\n=== Test Case 4: Revoke r3 from r2 ===");

    let revoke1_updates =
        Capability::revoke_memory_child(&dom2, r2_h_in_dom2, r3_h_in_dom1).unwrap();
    println!("✓ Revoked r3 from r2 (updates: {})", revoke1_updates.len());
    println!("✓ Dom1 access to memory unchanged after revoking r3");

    // ================================================================
    // Revoke r2 from r1 (r1 is in Dom1's table; r2_h_in_dom1 is r2's SubHandle)
    // ================================================================
    println!("\n=== Test Case 5: Revoke r2 from r1 ===");

    let revoke2_updates =
        Capability::revoke_memory_child(&dom1, r1_h_in_dom1, r2_h_in_dom1).unwrap();
