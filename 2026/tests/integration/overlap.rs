...
fn test_alias_cannot_overlap_carved_child() {
    ...
    let result = Capability::alias_memory(&root, mem_root_h, overlap_access);
...
fn test_carve_cannot_overlap_carved_child() {
    ...
    let overlap_access = Access::new(0x1000, 0x1000, Rights::RWX);
    let result = Capability::carve_memory(&root, mem_root_h, overlap_access);
    assert!(result.is_err());
    assert!(matches!(result, Err(CapaError::InvalidAccess)));
}
...
fn test_non_overlapping_operations_succeed() {
    ...
    let aliased = Capability::alias_memory(&root, mem_root_h, access2);
    ...
    let carved2 = Capability::carve_memory(&root, mem_root_h, access3);
...
fn test_nested_carve_overlap_validation() {
    ...
    let result = Capability::alias_memory(&root, parent_h, overlap_access);
    assert!(result.is_err());
    assert!(matches!(result, Err(CapaError::InvalidAccess)));
}
