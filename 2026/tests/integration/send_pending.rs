...
fn send_then_accept() {
    ...
    assert!(
        ...
        "handle must be frozen after send"
    );
    ...
    assert!(
        sender.read().data.get_memory_capability(1).is_none(),
        "handle must be gone from sender"
    );
    assert!(
        !sender.read().data.is_memory_handle_frozen(1),
    ...
    assert!(receiver
        .read()
        .data
        .get_memory_capability(new_handle)
        .is_some());
    ...
    let has_unmap = updates
        .updates()
        .iter()
        .any(|u| matches!(u, Update::ChangeRights { domain, rights, .. } if *domain == sender_id && *rights == Rights::NONE));
    let has_map = updates
        .updates()
        .iter()
...
fn send_then_reject() {
    ...
    assert!(
        sender.read().data.get_memory_capability(1).is_some(),
        "handle must still be in sender's table"
    );
    assert!(
        !sender.read().data.is_memory_handle_frozen(1),
        "handle must not be frozen after reject"
    );
...
fn frozen_handle_refuses_ops() {
    ...
    sender
        .write()
        .data
        .add_domain_capability(1, Arc::downgrade(&receiver1));
    sender
        .write()
        .data
...
fn revoke_parent_cancels_pending() {
    ...
    let (child_handle, child_sub, _) =
        Capability::<Domain>::carve_memory(&sender, 1, Access::new(0x1000, 0x1000, Rights::RW))
            .unwrap();
...
fn accept_gives_independent_handles() {
    ...
    sender
        .write()
        .data
        .add_memory_capability(1, Arc::downgrade(&cap1));
    ...
    sender
        .write()
        .data
        .add_memory_capability(2, Arc::downgrade(&cap2));
