...
fn test_display_unmarshall() {
    ...
    for i in 0..32 {
        engine
            .set(
                ...
                FieldType::InterruptVisibility,
                i,
                VectorVisibility::ALLOWED.bits() as usize,
            )
            .unwrap();
        engine
            .set(td0.clone(), child_td, 0, FieldType::InterruptRead, i, 0)
    ...
    engine
        .set(
            ...
            FieldType::InterruptVisibility,
            32,
            VectorVisibility::VISIBLE.bits() as usize,
        )
        .unwrap();
...
|cores: 0x1
|mon.api: 0x0
|vec0-31: ALLOWED, r: 0x0, w: 0x0
|vec32: VISIBLE, r: 0xffffffffffffffff, w: 0xffffffffffffffff
|vec33-255: NOT REPORTED, r: 0xffffffffffffffff, w: 0xffffffffffffffff
"#;
...

    {
        let child = td0
            ...
            .as_domain()
            .unwrap();
        let reconstruct = Domain::from_string(display).unwrap();
        assert_eq!(reconstruct.status, child.borrow().data.status);
        assert_eq!(
            reconstruct.policies.cores,
            child.borrow().data.policies.cores
        );
        assert_eq!(reconstruct.policies.api, child.borrow().data.policies.api);
        for i in 0..NB_INTERRUPTS {
            assert_eq!(
                reconstruct.policies.interrupts.vectors[i],
                child.borrow().data.policies.interrupts.vectors[i]
            );
        }
    }
}
