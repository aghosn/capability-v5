...
| `QEMU_TPM` | `0` | Set to `1` to attach a software TPM 2.0 (swtpm via `tpm-crb`). Auto-starts swtpm if not already running. Install: `sudo apt install swtpm swtpm-tools`. |
...
> **Note:** Uses `tpm-crb` (Command Response Buffer) instead of `tpm-tis` to avoid
> ISA interrupt conflicts with `kernel-irqchip=split`.
