...
fn main() {
    ...
    let task = match std::env::args().nth(1) {
        ...
        None => {
            ...
            eprintln!("tasks: iso | run-qemu | themis-debug | gdb | fetch-dom0 | run-dom0 | setup-limine | resize-disk | aarch64-iso | aarch64-themis | aarch64-direct");
            std::process::exit(1);
        }
    };
    ...
    if !script.exists() {
        ...
        eprintln!("available tasks: iso | run-qemu | themis-debug | gdb | fetch-dom0 | run-dom0 | setup-limine | resize-disk | aarch64-iso | aarch64-themis | aarch64-direct");
        std::process::exit(1);
    }
