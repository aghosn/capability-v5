...
**Status**: **M5c complete** (2026-06). Full initramfs boot on QEMU aarch64:
boot descriptor system (TDBS), FDT patching, Linux unpacks initramfs and runs
`/init`. Direct-assign interrupt model (IMO=FMO=0). Next: M6 (full distro boot).
...
| **M5c** | Boot descriptor, FDT patching, initramfs boot | ✅ Done (2613117) |
| **M6** | Full distro boot (virtio-blk, Ubuntu ARM64 cloud image) | 🔜 Next |
| **M7** | SMMUv3 (stretch) | Planned |
...
### Files created/modified (M5c)

| File | What |
|------|------|
| `capavisor/src/arch/aarch64/boot_descriptor.rs` | TDBS boot descriptor parser → `ModuleInfo` |
| `capavisor/src/arch/aarch64/fdt_patch.rs` | FDT binary patcher: adds initrd-start/end to /chosen |
| `capavisor/linker-aarch64-direct.ld` | `__image_end` symbol (2M-aligned after BSS) |
| `capavisor/src/main.rs` | Boot descriptor parsing, `find_module()` for kernel/initrd |
| `scripts/aarch64-direct.sh` | Dynamic memory layout from ELF, TDBS blob generation |
| `scripts/fetch-aarch64-kernel.sh` | Downloads Ubuntu ARM64 kernel .deb, extracts Image |

### Key learnings (M5c)

- **Boot descriptor system (TDBS)**: QEMU script reads `__image_end` from ELF via
  `nm`, computes all addresses dynamically, generates a binary descriptor blob
  listing modules. Capavisor uses `find_module("dom0-kernel")` — same pattern as
  x86/Limine. No hardcoded addresses in Rust.
- **FDT header offsets**: `size_dt_strings` is at 0x20, `size_dt_struct` at 0x24.
  Getting these wrong produces "Cannot locate property" errors in Linux.
- **FDT patch ordering**: Shift struct block first, THEN update strings_off, THEN
  add strings. Writing strings before shift corrupts nameoff values.
- **BSS extends far**: `__bss_end` at 0x40532038. Previous `GUEST_RAM_START` at
  0x40200000 was inside BSS — latent bug. Fixed via `__image_end`.
...
## 10. Boot Path Architecture: QEMU vs Bare-Metal

### The EL2 problem

Themis must run at EL2 (hypervisor exception level) to configure Stage-2 page tables,
trap guest exceptions, and manage the GIC virtual interface. On ARM, you **cannot**
transition from EL1 to EL2 — the architecture explicitly forbids privilege escalation.
You must **start** at EL2.

### Two boot paths, one module abstraction

| Environment | Boot method | EL at entry | Module source |
|---|---|---|---|
| **QEMU (development)** | `-kernel` direct-boot, `virtualization=on` | **EL2** ✅ | TDBS boot descriptor (generated by launch script) |
| **Bare-metal (production)** | UEFI firmware → Limine bootloader | **EL2** ✅ | Limine `ModuleInfo` (same as x86) |

Both paths converge on the same `find_module()` abstraction (`guest/modules.rs`):

```
QEMU direct-boot:     script → TDBS blob → parse_descriptor() → ModuleInfo[]
Limine UEFI boot:     firmware → Limine → ModuleRequest → ModuleInfo[]
                                          ↓
                              find_module("dom0-kernel") → base, size
                              find_module("dom0-initrd") → base, size
```

### Why Limine works on bare-metal but not on QEMU

On **SBSA-compliant ARM servers** (Ampere Altra, AWS Graviton, etc.), the UEFI
firmware runs at EL3 (TF-A) and hands off to the OS/hypervisor at **EL2**. Since
Limine is loaded as a UEFI application, it inherits EL2 and passes it to the
loaded kernel. Limine's module system provides kernel, initrd, and any additional
blobs — exactly the same as x86.

On **QEMU**, the EDK2/OVMF firmware drops to **EL1** before loading EFI
applications. Limine therefore runs at EL1, and the loaded kernel starts at EL1.
There is no way to reclaim EL2 from EL1.

QEMU's `-kernel` flag with `virtualization=on` bypasses UEFI entirely and loads
the ELF directly at EL2 — this is why we use the direct-boot path for development.

### Bare-metal boot plan

When targeting real hardware, the boot flow will be:

1. **Build**: Produce a Limine UEFI ISO (`cargo aarch64-iso`) — same as M1
2. **Firmware**: SBSA UEFI loads Limine at EL2
3. **Limine**: Provides HHDM, memory map, module list (kernel, initrd, rootfs)
4. **Capavisor entry**: Detects EL2, reads modules via Limine protocol
5. **Boot dom0**: Same flow as QEMU path from `find_module()` onward

Key items needed for bare-metal:
- Verify Limine UEFI on real ARM hardware provides EL2 entry
- M1's Limine boot code (`arch/aarch64/boot.rs`) needs to be updated to perform
  EL2 setup (MMU, vectors, Stage-2) before guest entry
- Platform-specific device discovery (ACPI vs FDT — servers may use ACPI)
- SMMUv3 for IOMMU (device isolation on real hardware)
- Real GIC hardware variations (GICv3 with ITS, GICv4 for direct injection)

### Design principle

The module abstraction (`ModuleInfo` + `find_module()`) is the convergence point.
All boot-path-specific code lives in:
- `arch/aarch64/boot.rs` — Limine UEFI path (bare-metal)
- `arch/aarch64/boot_descriptor.rs` — TDBS path (QEMU direct-boot)

The capavisor boot flow after module discovery is identical regardless of path.

---

## 11. M6 — Full Distro Boot Plan

### Architecture advantage: direct-assign model

HCR_EL2 is configured with IMO=FMO=AMO=0 (direct-assignment). Physical interrupts
go directly to the guest at EL1 without trapping to EL2. The guest owns the GIC
hardware directly. This means:

- **Timer interrupts**: ARM generic timer fires at EL1 directly ✅
- **UART interrupts**: PL011 identity-mapped, guest handles directly ✅
- **Virtio interrupts**: MSI-X via GIC-ITS, guest handles directly ✅
- **No vGIC needed**: single-guest direct-assign avoids interrupt virtualization
- **No monitor loop needed**: capavisor only handles synchronous exceptions (HVC/SMC)

This is valid for the **single-guest** scenario. Multi-domain will require
IMO/FMO=1, vGIC emulation, and a proper monitor loop.

### Steps

1. **Verify interrupt delivery**: Replace minimal init with busybox shell in
   initramfs. If we get a working shell prompt, timer and console input work.

2. **Add virtio-blk disk**: Download Ubuntu ARM64 cloud image. Add `-drive` and
   `-device virtio-blk-pci` to `aarch64-direct.sh`. PCIe MMIO and ECAM are
   already Stage-2 mapped.

3. **Boot full distro**: Set `root=/dev/vda1 rw` in kernel cmdline (via FDT
   `/chosen/bootargs` patch or QEMU `-append`). Boot to systemd login prompt.

4. **Fetch script**: Create `fetch-aarch64-dom0.sh` (downloads Ubuntu ARM64
   cloud image, seeds cloud-init with user/password).

### What's already mapped (Stage-2)

| Region | IPA range | Purpose |
|---|---|---|
| Guest RAM | `__image_end`..0x80000000 | Dynamic, ~1 GiB |
| GIC (GICD+GICR) | 0x08000000..0x08100000 | Interrupt controller |
| PL011 UART | 0x09000000..0x09001000 | Console |
| Virtio MMIO | 0x0A000000..0x0A200000 | Virtio transports |
| PCIe ECAM | 0x4010000000..0x4020000000 | PCI config space |
| PCIe low MMIO | 0x10000000..0x40000000 | PCI BAR (low) |
| PCIe high MMIO | 0x8000000000..0x10000000000 | PCI BAR (high) |

Virtio-blk-pci should work out of the box since PCIe config space and MMIO
ranges are already identity-mapped as device memory.
