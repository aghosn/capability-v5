...
fn test_meta_send_requires_exclusive() {
    ...
    let (alias_h, _, _)= Capability::<Domain>::alias(&platform, &sender, 1, Access::new(0x0, 0x1000, Rights::RWX))
    .unwrap();
    ...
    let result = Capability::<Domain>::send(&platform, &sender, alias_h, 10, Attributes::from_bits(Attributes::META));
    assert_eq!(
        result.unwrap_err(),
...
fn test_meta_cannot_be_re_sent() {
    ...
    let result = Capability::<Domain>::send(&platform, &middle, meta_handle, 3, Attributes::NONE);
    assert_eq!(
        result.unwrap_err(),
        CapaError::PermissionDenied,
...
fn test_meta_cannot_be_carved() {
    ...
    let result = Capability::<Domain>::carve(&platform, &receiver, meta_handle, Access::new(0x0, 0x100, Rights::R));
    assert_eq!(
        result.unwrap_err(),
        CapaError::PermissionDenied,
...
fn test_meta_cannot_be_aliased() {
    ...
    let meta_handle = *receiver.read().data.memory_capabilities.keys().next().unwrap();
    let result = Capability::<Domain>::alias(&platform, &receiver, meta_handle, Access::new(0x0, 0x100, Rights::R));
    assert_eq!(
        result.unwrap_err(),
        CapaError::PermissionDenied,
...
fn test_meta_revocation_triggers_domain_revoke() {
    ...
    let (carved_h, carved_sub, _) = Capability::<Domain>::carve(&platform, &root, 1, Access::new(0x0, 0x1000, Rights::RWX))
    .unwrap();
    ...
    Capability::<Domain>::send(&platform, &root, carved_h, 5, Attributes::from_bits(Attributes::META))
    .unwrap();

    // Revoke the META cap from root (by sub_handle, since it was sent away).
    let updates = Capability::<Domain>::revoke(&platform, &root, 1, carved_sub).unwrap();
...
fn test_meta_send_rejects_region_with_children() {
    ...
    let (_carved_h, _carved_sub, _) = Capability::<Domain>::carve(&platform, &sender, 1, Access::new(0x0, 0x1000, Rights::RWX))
    .unwrap();
    ...
    let result = Capability::<Domain>::send(&platform, &sender, 1, 10, Attributes::from_bits(Attributes::META));
    assert_eq!(
        result.unwrap_err(),
        CapaError::PermissionDenied,
...
fn test_meta_sealed_send_and_accept_no_mmu_update() {
    ...
    let (new_handle, updates) =
        Capability::<Domain>::accept(&platform, &receiver, pending_ids[0]).unwrap();
