...
fn loom_e2e_concurrent_sends() {
    loom::model(|| {
        ...
        let dh_a = Capability::<Domain>::create(&NullPlatform::new(), &dom, DomainPolicy::new_root(1))
            .expect("setup: create recv_a").0;
        let dh_b = Capability::<Domain>::create(&NullPlatform::new(), &dom, DomainPolicy::new_root(1))
        ...
        let ta = thread::spawn(move || {
            execute_shared(&opl, &ul_a, &st, || {
                let batch = Capability::<Domain>::send(&NullPlatform::new(), &d, h_c1, dh_a, Attributes::NONE)?;
                Ok(((), batch))
            })
        });
        ...
        let tb = thread::spawn(move || {
            execute_shared(&opl, &ul_b, &st, || {
                let batch = Capability::<Domain>::send(&NullPlatform::new(), &d, h_c2, dh_b, Attributes::NONE)?;
                Ok(((), batch))
            })
        });
...
fn loom_e2e_accept_race() {
    loom::model(|| {
        ...
        let dh_recv = Capability::<Domain>::create(&NullPlatform::new(), &sender, DomainPolicy::new_root(1))
            .expect("setup: create recv").0;
        Capability::<Domain>::seal(&NullPlatform::new(), &sender, dh_recv)
        ...
        let (h_c1, _sub1, _) = Capability::<Domain>::carve(
            &NullPlatform::new(), &sender,
            h_root,
            Access::new(0x0, 0x1000, Rights::RW),
        )
        .expect("setup: carve c1");
        Capability::<Domain>::send(&NullPlatform::new(), &sender, h_c1, dh_recv, Attributes::NONE)
        ...
        let ta = thread::spawn(move || {
            execute_shared(&opl, &ul_a, &st, || {
                let (handle, batch) = Capability::<Domain>::accept(&NullPlatform::new(), &r, pending_id)?;
                Ok((handle, batch))
            })
        });
...
fn loom_e2e_revoke_child_vs_send() {
    loom::model(|| {
        ...
        Capability::<Domain>::send(&NullPlatform::new(), &dom, h_c1, dh_a, Attributes::NONE)
        ...
        let ta = thread::spawn(move || {
            execute_exclusive(&opl, &ul_a, &st, || {
                let batch = Capability::<Domain>::revoke(&NullPlatform::new(), &d, h_root, sub_c1)?;
                Ok(((), batch))
            })
        });
...
fn loom_e2e_domain_revoke_vs_mem_send() {
    loom::model(|| {
        ...
        let h_child = Capability::<Domain>::create(&NullPlatform::new(), &dom, DomainPolicy::new_root(1))
        ...
        let dh_r = Capability::<Domain>::create(&NullPlatform::new(), &dom, DomainPolicy::new_root(1))
        ...
        let ta = thread::spawn(move || {
            execute_exclusive(&opl, &ul_a, &st, || {
                let batch = Capability::<Domain>::revoke_domain(&NullPlatform::new(), &d, h_child)?;
                Ok(((), batch))
            })
        });
        ...
        let tb = thread::spawn(move || {
            execute_shared(&opl, &ul_b, &st, || {
                let batch = Capability::<Domain>::send(&NullPlatform::new(), &d, h_c1, dh_r, Attributes::NONE)?;
                Ok(((), batch))
            })
        });
...
fn loom_e2e_send_to_domain_being_revoked() {
    loom::model(|| {
        ...
        let tb = thread::spawn(move || {
            execute_shared(&opl, &ul_b, &st, || {
                let batch = Capability::<Domain>::send(&NullPlatform::new(), &d, h_c1, h_child, Attributes::NONE)?;
                Ok(((), batch))
            })
        });
...
fn loom_e2e_two_cores_race_send_same_cap() {
    loom::model(|| {
        ...
        let tb = thread::spawn(move || {
            execute_shared(&opl, &ul_b, &st, || {
                let batch = Capability::<Domain>::send(&NullPlatform::new(), &d, h_c1, dh_b, Attributes::NONE)?;
                Ok(((), batch))
            })
        });
...
fn loom_e2e_send_vs_revoke_same_cap() {
    loom::model(|| {
        ...
        let dh_recv = Capability::<Domain>::create(&NullPlatform::new(), &dom, DomainPolicy::new_root(1))
            .expect("setup: create recv").0;
        let recv = {
            let r = dom.read();
            r.data.get_domain_capability(dh_recv).unwrap().upgrade().unwrap()
        ...
        let ta = thread::spawn(move || {
            execute_shared(&opl, &ul_a, &st, || {
                let batch = Capability::<Domain>::send(&NullPlatform::new(), &d, h_c1, dh_recv, Attributes::NONE)?;
                Ok(((), batch))
            })
        });
...
fn loom_e2e_domain_revoke_with_memory_vs_send() {
    loom::model(|| {
        ...
        Capability::<Domain>::send(&NullPlatform::new(), &dom, h_c1, h_child, Attributes::NONE)
        ...
        let tb = thread::spawn(move || {
            execute_shared(&opl, &ul_b, &st, || {
                let batch = Capability::<Domain>::send(&NullPlatform::new(), &d, h_c2, dh_r, Attributes::NONE)?;
                Ok(((), batch))
            })
        });
